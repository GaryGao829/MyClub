...
/** The ratio of rx/ry in the 3D pyramid opening
 * @private */
...
DvtChartPyramidSlice.prototype._getPath = function() {
  ...
  var center3DOffset = this._3dRatio * 0.2 * 0.98 * this._pyramidWidth;
  ...
  var topEdgeWidth = (1 - this._3dRatio * 0.2) * 0.98 * this._pyramidWidth * (heightToTopEdge / this._pyramidHeight);
  var gapAdjustedHeightToBottom = this._numDrawnSeries == 0 ? heightToBottomEdge : heightToBottomEdge - this._gap;
  var bottomEdgeWidth = (1 - this._3dRatio * 0.2) * 0.98 * this._pyramidWidth * (gapAdjustedHeightToBottom / this._pyramidHeight);
  ...
  if (this._3dRatio > 0) {
    ...
    var bottomEdge3DOffset = this._3dRatio * 0.2 * bottomEdgeWidth * Math.sqrt(2) / 2;
    var topEdge3DOffset = this._3dRatio * 0.2 * topEdgeWidth * Math.sqrt(2) / 2;

    // 3D Top
    pathCommands += dvt.PathUtils.lineTo(topRightCoords[0] + (isBiDi ? -1 : 1) * Math.max(topEdge3DOffset, 1), topRightCoords[1] - Math.max(topEdge3DOffset, 0.5));
    pathCommands += dvt.PathUtils.lineTo(topLeftCoords[0] + (isBiDi ? -1 : 1) * Math.max(topEdge3DOffset, 1), topLeftCoords[1] - Math.max(topEdge3DOffset, 0.5));
    pathCommands += dvt.PathUtils.lineTo(topLeftCoords[0], topLeftCoords[1]);
    pathCommands += dvt.PathUtils.lineTo(topRightCoords[0], topRightCoords[1]);

    // 3D Side
    pathCommands += dvt.PathUtils.lineTo(topRightCoords[0] + (isBiDi ? -1 : 1) * Math.max(topEdge3DOffset, 1), topRightCoords[1] - Math.max(topEdge3DOffset, 0.5));
    pathCommands += dvt.PathUtils.lineTo(bottomRightCoords[0] + (isBiDi ? -1 : 1) * bottomEdge3DOffset, bottomRightCoords[1] - bottomEdge3DOffset);
  ...
  return {'slice': pathCommands, 'sliceBounds': sliceBounds};
...
DvtChartPyramidSlice.prototype.setSelected = function(selected) {
  ...
  var w = dims.w;
  if (selected) {
    this.setScaleX((w - displacement) / w);
    this.setTranslateX(Math.ceil(displacement / 2) + displacement / w * dims.x);
  } else {
    this.setScaleX(1);
