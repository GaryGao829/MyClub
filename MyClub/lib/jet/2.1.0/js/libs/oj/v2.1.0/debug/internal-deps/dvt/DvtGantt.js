...
dvt.Gantt.prototype.triggerAnimations = function()
{
  ...
  // Parallel(Remove Rows, Add Rows, Sequential(remove elements, move elements, add elements))
  ...
  else if (this._animationMode === 'dataChange')
  {
    ...
    var removeRowsPlayables = [removeRowsFakeRmAnimPlayable, removeRowsRmAnimPlayable];
    var removeRowsPlayable = new dvt.ParallelPlayable(context, removeRowsPlayables, this._animationDuration, 0);
    ...
    var addRowsPlayables = [this.addRowsMvRowsPlayable, addRowsFrAnimPlayable];
    var addRowsPlayable = new dvt.ParallelPlayable(context, addRowsPlayables, this._animationDuration, 0);
    ...
    var removeContentPlayable = new dvt.ParallelPlayable(context, rmAnimPlayable, this._animationDuration, 0);
    ...
    var addContentPlayables = [this.newTaskDurationPlayable, addFrAnimPlayable];
    var addContentPlayable = new dvt.ParallelPlayable(context, addContentPlayables, this._animationDuration, 0);

    var sequentialSubPlayables = [];
    if (this.rmAnimElems.length > 0)
      sequentialSubPlayables.push(removeContentPlayable);
    sequentialSubPlayables.push(mvPlayable);
    if (this.frAnimElems.length > 0)
      sequentialSubPlayables.push(addContentPlayable);

    var sequentialPlayables = new dvt.SequentialPlayable(context, sequentialSubPlayables);
    var overallPlayable = new dvt.ParallelPlayable(context, [removeRowsPlayable, addRowsPlayable, sequentialPlayables], this._animationDuration, 0);
 ...
 * @override
 */
dvt.Gantt.prototype.handleZoomWheel = function(newLength, time, compLoc, triggerViewportChangeEvent, event)
{
  if (event && !event.ctrlKey && !dvt.Agent.isTouchDevice())
  {
    var wheelDelta = event.wheelDelta * 10; // TODO: This is arbitrary right now. Need to figure out a good deltaY value
    this.panBy(0, -wheelDelta);
  }
  else
  {
...
DvtGanttRenderer._renderZoomControls = function(gantt)
{
  ...
  var timeAxis = gantt.getTimeAxis();

  var imageSize = 16;
  var backgroundColor = DvtGanttStyleUtils._DEFAULT_ZOOM_CONTROL_BACKGROUND_COLOR;
  var borderColor = DvtGanttStyleUtils._DEFAULT_ZOOM_CONTROL_BORDER_COLOR;

  var upState = dvt.TransientButton.getStateFromURL(context, gantt._resources['zoomIn'], imageSize, imageSize, backgroundColor, borderColor);
  var overState = dvt.TransientButton.getStateFromURL(context, gantt._resources['zoomIn_h'], imageSize, imageSize, backgroundColor, borderColor);
  var downState = dvt.TransientButton.getStateFromURL(context, gantt._resources['zoomIn_a'], imageSize, imageSize, backgroundColor, borderColor);
  var disabledState = dvt.TransientButton.getStateFromURL(context, gantt._resources['zoomIn_d'], imageSize, imageSize, backgroundColor, borderColor);

  if (gantt.zoomin == null)
  {
    gantt.zoomin = new dvt.TransientButton(context, upState, overState, downState,
        disabledState, gantt.EventManager, gantt.EventManager.HandleZoomInClick);
    // In order for tooltips to show up, we need to associate the buttons through the event manager
    gantt.EventManager.associate(gantt.zoomin, gantt.zoomin);
  }
  else
  {
    gantt.zoomin.setUpState(upState);
    gantt.zoomin.setOverState(overState);
    gantt.zoomin.setDownState(downState);
    gantt.zoomin.setDisabledState(disabledState);
  }

  upState = dvt.TransientButton.getStateFromURL(context, gantt._resources['zoomOut'], imageSize, imageSize, backgroundColor, borderColor);
  overState = dvt.TransientButton.getStateFromURL(context, gantt._resources['zoomOut_h'], imageSize, imageSize, backgroundColor, borderColor);
  downState = dvt.TransientButton.getStateFromURL(context, gantt._resources['zoomOut_a'], imageSize, imageSize, backgroundColor, borderColor);
  disabledState = dvt.TransientButton.getStateFromURL(context, gantt._resources['zoomOut_d'], imageSize, imageSize, backgroundColor, borderColor);

  if (gantt.zoomout == null)
  {
    gantt.zoomout = new dvt.TransientButton(context, upState, overState, downState,
        disabledState, gantt.EventManager, gantt.EventManager.HandleZoomOutClick);
    // In order for tooltips to show up, we need to associate the buttons through the event manager
    gantt.EventManager.associate(gantt.zoomout, gantt.zoomout);
  }
  else
  {
    gantt.zoomout.setUpState(upState);
    gantt.zoomout.setOverState(overState);
    gantt.zoomout.setDownState(downState);
    gantt.zoomout.setDisabledState(disabledState);
  }

  gantt.zoomin.setTooltip(dvt.Bundle.getTranslatedString(dvt.Bundle.UTIL_PREFIX, 'ZOOM_IN', null));
  gantt.zoomout.setTooltip(dvt.Bundle.getTranslatedString(dvt.Bundle.UTIL_PREFIX, 'ZOOM_OUT', null));
  gantt.zoomin.hide();
  gantt.zoomout.hide();

  if (dvt.TimeAxis.supportsTouch())
  {
    dvt.ToolkitUtils.setAttrNullNS(gantt.zoomin.getElem(), 'role', 'button');
    dvt.ToolkitUtils.setAttrNullNS(gantt.zoomin.getElem(), 'aria-label', dvt.Bundle.getTranslatedString(dvt.Bundle.UTIL_PREFIX, 'ZOOM_IN', null));
    dvt.ToolkitUtils.setAttrNullNS(gantt.zoomout.getElem(), 'role', 'button');
    dvt.ToolkitUtils.setAttrNullNS(gantt.zoomout.getElem(), 'aria-label', dvt.Bundle.getTranslatedString(dvt.Bundle.UTIL_PREFIX, 'ZOOM_OUT', null));
  }
  ...
  gantt.zoomin.setTranslateX(transX);
  gantt.zoomout.setTranslateX(transX);
  ...
  gantt.zoomin.setTranslateY(yOffset);
  gantt.zoomout.setTranslateY(yOffset + DvtGanttStyleUtils._DEFAULT_ZOOM_CONTROL_DIAMETER + DvtGanttStyleUtils._DEFAULT_ZOOM_CONTROL_SPACING);

  if (gantt.getAxisPosition() == 'top')
  {
    gantt.zoomin.setTranslateY(transY);
    gantt.zoomout.setTranslateY(transY + DvtGanttStyleUtils._DEFAULT_ZOOM_CONTROL_DIAMETER + DvtGanttStyleUtils._DEFAULT_ZOOM_CONTROL_SPACING);
  }
  else
  {
    gantt.zoomin.setTranslateY(transY - DvtGanttStyleUtils._DEFAULT_ZOOM_CONTROL_DIAMETER * 2 - DvtGanttStyleUtils._DEFAULT_ZOOM_CONTROL_SPACING);
    gantt.zoomout.setTranslateY(transY - DvtGanttStyleUtils._DEFAULT_ZOOM_CONTROL_DIAMETER);
  }

  if (gantt.zoomin.getParent() != gantt._canvas)
    gantt._canvas.addChild(gantt.zoomin);
  if (gantt.zoomout.getParent() != gantt._canvas)
    gantt._canvas.addChild(gantt.zoomout);

  var contentLength = gantt.getContentLength();
  if (contentLength >= timeAxis.getMaxContentLength())
    gantt.disableZoomButton(true);
  if (gantt._canvasLength >= contentLength)
    gantt.disableZoomButton(false);
};
