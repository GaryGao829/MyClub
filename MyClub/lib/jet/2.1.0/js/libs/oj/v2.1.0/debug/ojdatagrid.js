...
{
    var activeCell, prevActiveCell, prop;
    if (this.m_active != null && this.m_active['type'] != 'header')
    {
        activeCell = this._getActiveElement();
        prop = this.getResources().isRTLMode() ? 'borderLeftStyle' :  'borderRightStyle';
        if (activeCell != null)
        {
            if (this._isLastRow(this.m_active['indexes']['row']))
            {
                activeCell['style']['borderBottomStyle'] = activeValue;
            }
            if (this._isLastColumn(this.m_active['indexes']['column']))
            {
                activeCell['style'][prop] = activeValue;
            }
        }
    }
    ...
    if (prevValue != null && this.m_prevActive != null && this.m_prevActive['type'] != 'header')
    {
        ...
        if (prevActiveCell != null)
        {
            ...
            {
                prevActiveCell['style']['borderBottomStyle'] = prevValue;
            }                
            if (this._isLastColumn(this.m_prevActive['indexes']['column']))
            {
                prevActiveCell['style'][prop] = prevValue;
            }       
        }
    }
};
...
DvtDataGrid.prototype.resizeGrid = function()
{
    ...
    this.setElementDir(rowEndHeader, rowHeaderWidth + databodyWidth, dir);
    ...
    this.setElementDir(colEndHeader, colHeaderHeight + databodyHeight, 'top');
...
{
    var borderStyle, borderWidth, lastHeader, colHeaderHeight, colHeaderWidth, widthCheck, heightCheck, scrollbarSize,
            colEndHeaderHeight, rowHeaderWidth, rowHeaderHeight, rowEndHeaderWidth, width, height, i, tags;
    ...
    borderWidth = this.getResources().isRTLMode() ? "borderLeftWidth" : "borderRightWidth";
    
    widthCheck = rowHeaderWidth + colHeaderWidth + rowEndHeaderWidth + 2 + (this.m_hasVerticalScroller ? scrollbarSize : 0) < width;
    heightCheck = colHeaderHeight + rowHeaderHeight + colEndHeaderHeight + 2 + (this.m_hasHorizontalScroller ? scrollbarSize : 0) < height;
    ...
    if (this.m_rowHeader != null && this.m_endRowHeader >= 0 && (!heightCheck || this.m_rowHeaderScrollbarSpacer != null) && this._isLastRow(this.m_endRowHeader))
    {
        ...
        lastHeader['style']['borderBottomStyle'] = 'none';
    }
    ...
    if (widthCheck && this.m_endRowEndHeader >= 0 )
    {
        ...
        {
            this.m_columnHeaderScrollbarSpacer['style'][borderWidth] = '1px';
        }
        if (this.m_bottomCorner != null)
        {
            this.m_bottomCorner['style'][borderWidth] = '1px';
        }
        tags = this.m_rowEndHeader.firstChild.childNodes;
        for (i = 0; i < tags.length; i++) 
        {
          tags[i]['style'][borderWidth] = '1px';
        }
    }
    ...
    if (heightCheck && this.m_endColEndHeader >= 0)
    {
        ...
        {
            this.m_rowHeaderScrollbarSpacer['style']['borderBottomWidth'] = '1px';
        }             
        if (this.m_bottomCorner != null)
        {
            this.m_bottomCorner['style']['borderBottomWidth'] = '1px';
        }        
        tags = this.m_colEndHeader.firstChild.childNodes;
        for (i = 0; i < tags.length; i++) 
        {
          tags[i]['style']['borderBottomWidth'] = '1px';
        }
    }    
};
...
DvtDataGrid.prototype.isCellFetchResponseInViewport = function(cellSet, cellRange)
{
    ...
    return ((rowStartPixel >= viewportTop && rowStartPixel <= viewportBottom) || (rowEndPixel >= viewportTop && rowEndPixel <= viewportBottom)) && 
             ((columnStartPixel >= viewportLeft && columnStartPixel <= viewportRight) || (columnEndPixel >= viewportLeft && columnEndPixel <= viewportRight));
};
...
DvtDataGrid.prototype.manageResizeScrollbars = function()
{
    var width, height, colHeader, rowHeader, databody, colHeaderHeight, columnHeaderWidth, rowHeaderWidth, rowHeaderHeight, 
            ...
            isDatabodyVerticalScrollbarRequired, scrollbarSize, dir,
    ...
    if (!this._databodyEmpty())
    {
        ...
        if (!isDatabodyVerticalScrollbarRequired && !isDatabodyHorizontalScrollbarRequired && this.m_hasHorizontalScroller && this.m_hasVerticalScroller)
        {
            ...
            databody['style']['overflow'] = "visible";
        }
        if ((isDatabodyVerticalScrollbarRequired && isDatabodyHorizontalScrollbarRequired && !this.m_hasHorizontalScroller && !this.m_hasVerticalScroller)
